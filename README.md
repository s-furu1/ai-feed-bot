...
ai-feed-bot は、AI関連RSSを収集し、Ollamaで要約し、X投稿下書きを生成して Slack `#ai-feed` で確認するためのアプリです。
...
- Slack `#ai-feed` 固定パネルでの採用 / 保留 / 破棄 / 再生成
- SQLite による永続化
- worker による定期実行の基盤
...
通常操作は Slack `#ai-feed` 固定パネルとボタンで行います。
slash command は主UIにしません。管理用の疎通確認として `/feed ping` だけを用意しています。
...
```bash
AI_FEED_ENABLE_SLACK=true python -m app.main
```
